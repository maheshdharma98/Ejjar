...
const rfqsAll: Array<{
  status: string;
}> = require('../../../shared/mock/rfqs.json');
const jobsAll: Array<{
  status: string;
}> = require('../../../shared/mock/jobs.json');
...
export default function ProfileScreen() {
  ...
  const MENU_ITEMS = [
    {
      icon: 'file-document-outline',
      label: isAr ? 'طلباتي' : 'My RFQs',
      color: colors.primary,
      bg: colors.primaryLight,
      onPress: () => navigation.navigate('MyRFQs'),
    },
    {
      icon: 'briefcase-outline',
      label: isAr ? 'أعمالي' : 'My Jobs',
      color: colors.success,
      bg: '#DCFCE7',
    ...
    {
      icon: 'bell-outline',
      label: isAr ? 'الإشعارات' : t('notifications.title'),
      color: colors.warning,
      bg: colors.warningLight,
